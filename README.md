...
To address this we reprocessed our data and cut out all of the non-tournament teams, then re-ran some of the models.  Because we had not yet realized that we had categorical target data, our model metrics continued to crash, and so did our models.  For example, when running neural networking with keras, we obtained negative loss values in the 10-100k range.  
